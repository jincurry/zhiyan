...
import { esc, isSafeLink, dayKey } from './util.js';
import { toggleTodo } from './markdown.js';
import { drawTimeline } from './timeline.js';
import { drawSidebar, refreshTags, allTags, tagCountSnapshot } from './sidebar.js';
import { refreshStats, drawStats, setTab, rollRecall, setTagCounts } from './rail.js';
...
async function refresh() {
  await Promise.all([reload(), refreshTags(), refreshStats()]);
...
async function save() {
  ...
  await api.upsertMemo({ id: editing, text, source: 'app' });
  ed.clearComposer();
  await refresh();
...
async function toggleTodoAt(id, index) {
  ...
  await api.upsertMemo({ id, text: toggleTodo(memo.text, index) });
  await refresh();
}
...
async function exportMarkdown() {
  const all = await api.listMemos({ view: 'all', sort: 'new' });
  const body = all
    .map((m) => {
...
const ACTIONS = {
  // 标题栏
  'win-min': () => api.windowMinimize(),
  'win-max': () => api.windowToggleMaximize(),
  'win-close': () => api.windowHide(),
  theme: toggleTheme,
  ...
  'memo-pin': async (_, el) => {
    ...
    const memo = byId(id);
    if (!memo) return;
    await api.setPinned(id, !memo.pinned);
    await refresh();
  ...
  'memo-restore': async (_, el) => {
    await api.restoreMemo(memoIdOf(el));
    await refresh();
    ov.toast('已恢复');
  },
  'memo-purge': (_, el) => {
...
document.addEventListener('click', (e) => {
  ...
  if (link) {
    e.preventDefault();
    // 渲染层已经挡过一道，这里再挡一道：两层都要有
    if (isSafeLink(link.href)) api.openExternal(link.href);
    return;
  }
...
document.addEventListener('keydown', (e) => {
  ...
  if (e.ctrlKey && e.altKey && (e.code === 'Space' || e.key === ' ')) { e.preventDefault(); api.quickToggle(); }
...
palette.initPalette({
  ...
  commands: [
    ...
    { label: '速记浮窗', icon: '▭', kbd: 'Ctrl+Alt+Space', run: () => api.quickToggle() },
