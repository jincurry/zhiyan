...
async function loadContext() {
  try {
    const { tags: counts } = await store.stats();
    ...
    $('flTags').innerHTML = top
      ...
      .join('');
    memos = await store.listMemos({ view: 'all', sort: 'new' }, null, 200);
  } catch {
...
async function save() {
  ...
  if (!text) return;
  await store.upsertMemo({ text, source: 'quick' });
  ta.value = '';
  refresh();
  close();
}
...
function close() {
  if (platform.inTauri) platform.quickToggle();
}
...
ta.addEventListener('keydown', async (e) => {
  const ed = await import('./editor.js');
  // 组合串未上屏时不响应快捷键（§6.2）：打拼音时的回车是选词，不是存下
